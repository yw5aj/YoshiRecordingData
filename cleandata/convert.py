...
class CleanFiber:
    ...
    def get_animal_info(self):
        with open('./notes/ModelGfpInfo0729.csv', 'r') as f:
            animal_info_splitted = re.findall(self.mat_filename+r'.+', f.read())[0].split(',')
    ...
    def get_mat_data(self):
        self.mat_data = loadmat('./rawData/finalSAI/'+self.mat_filename)
        self.stim_block = []
        i = 0
        while True:
            try:
                self.stim_block.append(self.mat_data['COUT_PUT_F'+str(self.animal_info['unitid'])+'0'+str(i+1)].shape[1])
                i += 1
            except:
                break
    ...
    def sort_traces(self):
        ...
        for i, j_num in enumerate(self.stim_block):
            for j in range(j_num):
                traces_dict = {'raw_force': self.mat_data['COUT_PUT_F'+str(self.animal_info['unitid'])+'0'+str(i+1)][:, j],
                               'raw_displ': self.mat_data['COUT_PUT_D'+str(self.animal_info['unitid'])+'0'+str(i+1)][:, j],
                               'spike_trace': self.mat_data['OUT_PUT_CS'+str(self.animal_info['unitid'])+'0'+str(i+1)][:, j],
                               'time': np.arange(self.mat_data['COUT_PUT_F'+str(self.animal_info['unitid'])+'0'+str(i+1)].shape[0])/self.fs}
                try:
                    traces_dict['raw_spike'] = self.mat_data['OUT_PUT_R'+str(self.animal_info['unitid'])+'0'+str(i+1)][:, j]
                except KeyError:
                    pass
                traces_dict['force'] = filtfilt(b, a, traces_dict['raw_force'])
                traces_dict['displ'] = filtfilt(b, a, traces_dict['raw_displ'])
                traces_dict['displ'] = traces_dict['displ'] - traces_dict['displ'][0]
                # Organize the spike firing rate and timings
                if np.any(traces_dict['spike_trace']):
                    traces_dict['spike_time'], traces_dict['spike_isi'], traces_dict['spike_fr'] = self._get_spike_data(traces_dict['spike_trace'])
                    traces_dict['spike_fr_trace'] = np.interp(traces_dict['time'], traces_dict['spike_time'], traces_dict['spike_fr'])
                else:
                    traces_dict['spike_fr_trace'] = np.zeros(traces_dict['time'].shape)
    ...
    def find_contact_by_force(self, threshold, pad):
        ...
        for stim_id, stim_traces_full in enumerate(self.traces_full):
            # try-except is used in case some force traces are too trivial & below threshold force
            try:
                contact_index = np.nonzero(stim_traces_full['force']>threshold)[0][0]
            except:
                continue
    ...
    def cut_traces(self, make_plot=False):
        self.traces = []
        for stim_id, stim_traces_full in enumerate(self.traces_full):
            contact_index = np.nonzero(stim_traces_full['displ']>=self.contact_pos)[0][0]
            max_force_index = stim_traces_full['force'].argmax()
            self.traces.append({
                'force': stim_traces_full['force'][contact_index:contact_index+6*self.fs], 
                'displ': stim_traces_full['displ'][contact_index:contact_index+6*self.fs] - stim_traces_full['displ'][contact_index], 
                'spike_trace': stim_traces_full['spike_trace'][contact_index:contact_index+6*self.fs], 
                'time': np.arange(6*self.fs)/self.fs, 
                'ramp_time': (max_force_index - contact_index)/self.fs,
                'static_force': stim_traces_full['force'][max_force_index+2*self.fs:max_force_index+int(4.5*self.fs)].mean(),
                'static_displ': stim_traces_full['displ'][max_force_index+2*self.fs:max_force_index+int(4.5*self.fs)].mean() - stim_traces_full['displ'][contact_index],
                'dynamic_force_rate': np.diff(stim_traces_full['force'][contact_index:max_force_index]).mean()*self.fs,
                'dynamic_displ_rate': np.diff(stim_traces_full['displ'][contact_index:max_force_index]).mean()*self.fs,
                })
            try:
                self.traces[-1]['raw_spike'] = stim_traces_full['raw_spike'][contact_index:contact_index+6*self.fs]
            except KeyError:
                pass
            self.traces[-1]['static_avg_fr'] = self._get_avg_fr(stim_traces_full['spike_trace'][max_force_index+int(2.*self.fs):max_force_index+int(4.5*self.fs)])
            self.traces[-1]['dynamic_avg_fr'] = self._get_avg_fr(stim_traces_full['spike_trace'][contact_index:max_force_index])
            if np.any(self.traces[-1]['spike_trace']):
                self.traces[-1]['spike_time'], self.traces[-1]['spike_isi'], self.traces[-1]['spike_fr'] = self._get_spike_data(self.traces[-1]['spike_trace'])
            if make_plot:
                fig, axs = plt.subplots(3, 1, figsize=(6.83, 9.19))    
                for i, item in enumerate(['displ', 'force', 'spike_trace']):
                    axs[i].plot(stim_traces_full['time'][contact_index-.5*self.fs:contact_index+6.5*self.fs], stim_traces_full[item][contact_index-.5*self.fs:contact_index+6.5*self.fs], '-k', color='.0')
                    axs[i].axvline(contact_index/self.fs, color='.0')
                    axs[i].axvline(contact_index/self.fs+6., color='.0')
                    axs[i].set_xlabel('Time (s)')
#                    axs[i].set_xlim(0, 8)
                axs[0].set_title('displ = %f, force = %f, missed spikes = %d' % (self.traces[-1]['static_displ'], self.traces[-1]['static_force'], stim_traces_full['spike_trace'][:contact_index].sum()))
                axs[0].set_ylabel('Displ. ($\mu$m)')
                axs[1].set_ylabel('Force (mN)')
                axs[2].set_ylabel('Spikes')
                axs[2].set_ylim(-2, 2)
                fig.savefig('./plots/traces/'+self.mat_filename[:-4]+'stim'+
...
def plot_specific_fibers(fiberList, fname='temp'):
    if not isinstance(fiberList, list):
        fiberList = [fiberList]
    fig, axs, static_dynamic_array = plot_static_dynamic(fiberList, 
        save_data=False, fname=fname)
...
#        label_text = '#'+str(fiber_id)
        ...
        axs[0].plot(static_displ_list, static_force_list, fmt, color=color, ms=6, label=label_text)
        axs[1].plot(static_displ_list, static_avg_fr_list, fmt, color=color, ms=6, label=label_text)
        axs[2].plot(static_force_list, static_avg_fr_list, fmt, color=color, ms=6, label=label_text)
#        axs[3].plot(static_displ_list, dynamic_avg_fr_list, fmt, color=color, ms=6, label='#'+str(fiber_id))
...
def main(save_data=False):
    """
    This is the main convert_data function. It will take in the force 
    threshold and the back-off distance, and saves a pickle file with all 
    organized data. Also, the function will output the essential information 
    to help quick modeling with ABAQUS, including: steady-state displ, forces, 
    ramp times & regulated ramp
    curve.
    """
    pass
...
#    fig, axs, static_dynamic_array, ramp_time_list, ramp_time_coeff, regulated_ramp_curve_list, median_regulated_ramp_curve, popt = main(save_data=True)
    # Clear all old plots
    """
    ...
    """
    cleanFiberList = []
    for filename in os.listdir('./rawData/finalSAI/'):
        if filename.endswith('.mat'):
            cleanFiberList.append(CleanFiber(filename, threshold=.25, pad=300., make_plot=False))
            print(filename + ' completed...')
    for i, cleanFiber in enumerate(cleanFiberList):
        cleanFiber.fiber_id = i
    fig, axs, static_dynamic_array = plot_static_dynamic(cleanFiberList, save_data=True)
    ...
    regulated_ramp_curve_list, median_regulated_ramp_curve, popt = extract_regulated_ramp_curve(cleanFiberList)
    ...
    finalCleanFiberList = [cleanFiber for cleanFiber in cleanFiberList if 
        cleanFiber.animal_info['mat_filename'] not in exclude_list]
    plot_specific_fibers(finalCleanFiberList, fname='6left')
