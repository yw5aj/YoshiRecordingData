...
#        label_text = '#'+str(fiber_id)
        ...
        axs[0].plot(static_displ_list, static_force_list, fmt,
                    color=color, ms=6, label=label_text)
        axs[1].plot(static_displ_list, static_avg_fr_list, fmt,
                    color=color, ms=6, label=label_text)
        axs[2].plot(static_force_list, static_avg_fr_list, fmt,
                    color=color, ms=6, label=label_text)
#        axs[3].plot(static_displ_list, dynamic_avg_fr_list, fmt,
#                    color=color, ms=6, label='#'+str(fiber_id))
...
if __name__ == '__main__':
    ...
    for filename in os.listdir('./rawData/finalSAI/'):
        if filename.endswith('.mat'):
            cleanFiberList.append(CleanFiber(filename, threshold=.25, pad=300.,
                                             make_plot=make_plot))
    ...
    finalCleanFiberList = [
        cleanFiber for cleanFiber in cleanFiberList if
